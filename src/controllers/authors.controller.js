...
const deleteAuthor = async (req, res) => {
  ...
  try {
    ...
    res.json({ message: "Autor eliminado con éxito", author: result.rows[0] });
  } catch (err) {
    console.error(err);
    if (err.code === "23503") {
      return res.status(400).json({ error: "No se puede eliminar el autor porque tiene publicaciones asociadas" });
    }
